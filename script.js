        ...
        for (let c = 0; c < 5; c++) {
            ...
            if (l === 2 && c === 2) {
                ...
                criar.addEventListener("click", () => {
                    criar.classList.toggle("marcado");
                });
            }
...
function verificarBingo() {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j < 5; j++) {
            const idx = i * 5 + j;
            grid[i][j] = criars[idx].classList.contains("marcado") || (i === 2 && j === 2);
        }
    }
    ...
    for (let i = 0; i < 5; i++)
        if (grid[i].every(Boolean)) bingo = true;

    for (let j = 0; j < 5; j++)
        if (grid.every(row => row[j])) bingo = true;
